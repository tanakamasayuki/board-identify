...
- すべての `Identification` に `transport_kind` を設定してください。これはボードではなく経路を名指すもので、ターゲットの手前にあるブリッジなら `uart`、ターゲット自身の USB ペリフェラルなら `usb`、デバッグプローブのシリアルインタフェースなら `probe` です。前 2 つは `usb_ids` の `transport_kind_for_port()` が sysfs から判定します。プローブは自分がプローブだと知っているので、自分で名乗ります。設定しないとそのポートは修飾付きのリンクを失い、主張の優先順位も最後になるため、2 経路で到達できるボードはもう一方のポートを指すための名前を失います。
