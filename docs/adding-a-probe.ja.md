...
- すべての `Identification` に `transport_kind` を設定してください。これはボードではなく経路を名指すもので、ターゲットの手前にあるブリッジなら `uart`、ターゲット自身の USB ペリフェラルなら `usb`、デバッグプローブのシリアルインタフェースなら `probe` です。前 2 つは `usb_ids` の `transport_kind_for_port()` が sysfs から判定します。プローブは自分がプローブだと知っているので、自分で名乗ります。これは名前の一部ではありません。2 つのポートが 1 つのボード名を名乗ったときにどちらが保持するかを決めるもので、設定しないとその主張の優先順位が最後になります。
