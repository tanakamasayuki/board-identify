...
### 2 経路で到達できるボードの名前が 1 つしかない、あるいは 1 つもない

オンボードのネイティブ USB と、同じ UART に付いた CH340 の両方を挿した ESP32-S3 は 2 つのポートで応答します。どちらのポートにも 1 つずつ名前が付きます。

```bash
ls -l /run/board-identify/by-id/ | grep e4b063b4a81c
# esp32-s3-e4b063b4a81c      -> /dev/ttyUSB2    CH340 経由
...
chip info  00 00 03 07 ...（5 回繰り返し）   UUID が 0000030700000307 になる
```

family バイトが正しいままなので、一見それらしく見えてしまいます。残りは 4 バイトの同じ語の繰り返しで、attach と detach を繰り返しても解消せず、その状態にあるどのボードからも同じでたらめな UUID が返ります。おかしいのはボードではなくプローブです。ターゲットの電源を入れ直しても解消しません。`board-identify` はこれを検出して読み直すので、追加の操作は不要です。

何で解消するか、その代償は何か:

| コマンド | 解消するか | ターゲットをリセットするか |
| --- | --- | --- |
| `81 0d 01 03` — 復旧に使っているもの | する | しない |
| `81 0b 01 01` — `wlink reset`、probe-rs の `ResetTarget` | する | する |
| `81 0d 01 ff` — detach | しない | しない |
| `81 0d 01 13` — リセットライン Low | しない | しない |
| ターゲットの電源を入れ直す | しない | — |

デバッグモジュールのスティッキーな `havereset` ビットで計測した結果です。手動で解消する場合、手が届くのはリセットのほうですが、接続時であれば問題ありません。
