...
class FakeHandle:
    ...
    def __init__(self, replies: dict[bytes, bytes], fail_on: bytes | None = None) -> None:
    ...
    def write(self, endpoint: int, payload: bytes, timeout: int) -> int:
        ...
        if self.fail_on is not None and bytes(payload) == self.fail_on:
            raise usb.core.USBError("boom")
        self._pending = self.replies.get(bytes(payload), b"")
        return len(payload)
