...
1 つのボードが 2 つのポートで同時に応答することもあります。オンボードのネイティブ USB と、同じ UART に付いた CH340 を持つ ESP32-S3 です。この場合、ポートごとに別の名前が付きます。
...
4. 各名前を、それを名乗るポートのうち最良のものへ落ち着かせ、`/run/board-identify/by-id/` 配下にアトミックなシンボリックリンクとして公開する。
