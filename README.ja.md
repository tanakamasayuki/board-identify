...
1 つのボードが 2 つのポートで同時に応答することもあります。オンボードのネイティブ USB と、同じ UART に付いた CH340 を持つ ESP32-S3 です。そこで各リンクは 2 通りで公開されます。ボードの名前そのものと、その名前にトランスポートを付けた、特定の経路を指す名前です。
...
ボード自身の USB は決して開きません。開けばボードが再起動し、ポートごと消えるからです。そのディスクリプタには eFuse MAC はありますがチップ名はないので、この経路はシリーズまでで止まります。`esp32-series` という名前は、無印 `esp32` の名前を借りるのではなく、チップが未確認であることを示すものです。同じチップの手前にあるブリッジはチップ名までを名乗ります。両方に入っている MAC が、同じ 1 枚のボードであることを示します。
...
4. `/run/board-identify/by-id/` 配下に、そのポート専用であるトランスポート付きの名前をアトミックに公開し、修飾なしの名前はそれを名乗るポートのうち最良のものへ落ち着かせる。
